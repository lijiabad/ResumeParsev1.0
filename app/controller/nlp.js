...
class NlpController extends Controller {
    async nameParse(result, input_text ,new_json) {
        ...
        const NAME = [];
         ...
         //             const cleanedWords = words.replace(/\s/g, ''); // 去除空格
         //             if (/^[\u4e00-\u9fa5]{2,3}$/.test(cleanedWords)){
         //                 const firstChar = cleanedWords.charAt(0);
         //                 if (baijiaxing.includes(firstChar)){
         //                     NAME.push(cleanedWords);
         //                 }
         //             }
         //         }
         //     }
    ...
    async ageParse(result, input_text, new_json) {
        ...
        //     for (let i = 0; i < wordsResult.length; i++){
        //         const words = wordsResult[i].words;
        //         const matches2 = words.match(regex2);
        //         if (matches2){
        //             for (let j = 0; j < matches2.length; j++){
        //                 const ageMatch = matches2[j].match(/\d{2}/);
        //                 if (ageMatch){
